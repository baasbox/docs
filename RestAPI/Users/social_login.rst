...
Returns: 
-  200 code with an empty response if the unlink procedure was successful, 
-  400 code if the user was not linked to specified social network, 
-  401 code (Unauthorized) if any of the mandatory header was missing, 
-  500 code if something on the server went wrong (i.e. the user was generated and it had only a connection with a social network)
